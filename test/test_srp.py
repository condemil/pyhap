...
class TestSrp(TestCase):
    def setUp(self):
        generate_salt_patch = patch('pyhap.srp.generate_salt', return_value=TEST_s)
        generate_private_key_patch = patch('pyhap.srp.generate_private_key', return_value=srp.to_int(TEST_b))

        with generate_salt_patch, generate_private_key_patch:
            self.srp = srp.Srp(USERNAME, PASSWORD)
        self.srp.compute_shared_session_key(TEST_A)

    def test_server_password_verifier(self):
        self.assertEqual(srp.to_bytes(self.srp.v), TEST_v)

    def test_server_public_key(self):
        self.assertEqual(self.srp.public_key, TEST_B)

    def test_random_scrambling_parameter(self):
        self.assertEqual(srp.to_bytes(self.srp.u), TEST_u)

    def test_premastered_secret(self):
        self.assertEqual(srp.to_bytes(self.srp.S), TEST_S)

    def test_session_key(self):
        self.assertEqual(self.srp.session_key, TEST_K)

    def test_session_key_client_proof(self):
        self.assertEqual(self.srp.M, TEST_M)

    def test_session_key_server_proof(self):
        self.assertEqual(self.srp.session_key_proof, TEST_H_AMK)

    def test_salt(self):
        self.assertEqual(self.srp.salt, TEST_s)

    def test_verify_proof(self):
        self.assertTrue(self.srp.verify_proof(TEST_M))

    def test_generate_salt(self):
        salt1 = srp.generate_salt()
        salt2 = srp.generate_salt()

        self.assertEqual(type(salt1), bytes)
        self.assertEqual(len(salt1), 16)
        self.assertNotEqual(salt1, salt2)

    def test_generate_private_key(self):
        private_key1 = srp.generate_private_key()
        private_key2 = srp.generate_private_key()

        self.assertEqual(type(private_key1), int)
